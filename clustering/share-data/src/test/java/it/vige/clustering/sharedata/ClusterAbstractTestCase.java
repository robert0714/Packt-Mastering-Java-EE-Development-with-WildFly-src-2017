 ...
 * <ul>
 * <li>before every test method, containers are started and deployments are
 * deployed via {@link #beforeTestMethod()}</li>
 * <li>after every method execution the deployments are undeployed via
 * {@link #afterTestMethod()}</li>
 * </ul>
 *
 * Should the test demand different node/deployment setup, the methods must be
 * overridden.
 *
 * Furthermore provides convenience methods for {@link NodeUtil} utility
 * ({@link #start(String...)}, {@link #deploy(String...)}, etc).
 *
 * @author Radoslav Husar
 * @version Jul 2013
 */
public abstract class ClusterAbstractTestCase implements ClusteringConstants {
	...
	/**
	 * Guarantees that prior to test method execution both containers are
	 * running and both deployments are deployed.
	 */
	@Before
	...
	/**
	 * Guarantees that all deployments are undeployed after the test method has
	 * been executed.
	 */
	@After
	@RunAsClient // Does not work, see https://issues.jboss.org/browse/ARQ-351
