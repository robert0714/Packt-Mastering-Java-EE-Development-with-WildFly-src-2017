...
import static it.vige.clustering.sharedata.singleton.HAServicePolicyActivator.SERVICE_NAME;
...
import it.vige.clustering.sharedata.singleton.HAServicePolicyActivator;
...
public class SingletonPolicyServiceTestCase extends ClusterAbstractTestCase {
	...
	public void testSingletonService(
		...
		try (CloseableHttpClient client = promiscuousCookieHttpClient()) {
			HttpResponse response = client.execute(new HttpGet(createURI(baseURL1, SERVICE_NAME, NODE_1)));
			try {
			...
			response = client.execute(new HttpGet(createURI(baseURL1, SERVICE_NAME, NODE_1)));
			try {
				assertEquals(SC_OK, response.getStatusLine().getStatusCode());
				assertEquals(NODE_1, response.getFirstHeader("node").getValue());
			} finally {
			...
			response = client.execute(new HttpGet(createURI(baseURL2, SERVICE_NAME, NODE_1)));
			try {
				assertEquals(SC_OK, response.getStatusLine().getStatusCode());
				assertEquals(NODE_1, response.getFirstHeader("node").getValue());
			} finally {
			...
			response = client.execute(new HttpGet(createURI(baseURL2, SERVICE_NAME, NODE_2)));
			try {
				assertEquals(SC_OK, response.getStatusLine().getStatusCode());
			...
			response = client.execute(new HttpGet(createURI(baseURL1, SERVICE_NAME, NODE_2)));
			try {
				assertEquals(SC_OK, response.getStatusLine().getStatusCode());
				assertEquals(NODE_2, response.getFirstHeader("node").getValue());
			} finally {
